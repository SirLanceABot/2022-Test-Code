...
public class Xbox extends Joystick
{
    ...
    public class RumbleEvent implements Comparable<RumbleEvent>
    {
        ...
        {
            return String.format("start=%f duration=%f leftPower=%f rightPower=%f", startTime, duration, leftPower, rightPower);
        }
    }
    ...
    private final int NUMBER_OF_AXES = 6;
    ...
    public void createRumbleEvent(double startTime, double duration, double leftPower, double rightPower)
    {
        ...
        double reEndTime = 0.0;
        int count = 0;

        for(RumbleEvent re: rumbleEvents)
        {
            reEndTime = re.startTime - re.duration;
            if(re.startTime >= startTime && startTime > reEndTime)
            {
                isNoOverlap = false;
                count = 1;
            }
            else if(re.startTime > endTime && endTime > reEndTime)
            {
                isNoOverlap = false;
                count = 2;
            }
            else if(startTime >= re.startTime && re.startTime > endTime)
            {
                isNoOverlap = false;
                count = 3;
            }
        ...
        {
            System.out.println("Rumble event overlap. Rumble event not created.");
            System.out.println(startTime + " " + duration + " " + count);
        }
    }
